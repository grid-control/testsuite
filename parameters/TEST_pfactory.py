...
class Test_SimpleParameterFactory:
	...
	>>> norm_ps_display(getSPF('A B, C2[D]'))
	switch(ZIP(cross(var('A'), var('B')), switch_placeholder('C2', 'D')), 'C2', 'D')
	>>> norm_ps_display(getSPF('A B, 3*C2[D]'))
	switch(ZIP(cross(var('A'), var('B')), truncate(switch_placeholder('C2', 'D'), 3)), 'C2', 'D')
	...
	>>> testSPF('VAR, FMT', VAR = '1 2 3', FMT = 'VAR + 4', FMT_type = 'transform')
	3
...
class Test_Subspace:
	"""
	>>> ps = testSubSpace("cross(var('A'), pspace('ps1'), pspace('modular', 'ps2'))", 'ModularParameterFactory')
	>>> ps
...
class Test_Scope:
	...
	TruncateParameterSource: truncate = 6
	  NullParameterSource:
	>>> try_catch(lambda: testScope({'parameters': {'parameters': '{}'}}), 'ParameterError', 'Recusive parameter space detected')
	caught
	...
	caught
	>>> try_catch(lambda: testScope({'parameters': {'parameters': '<x y>'}}), 'ParameterError', 'Error while creating')
	caught

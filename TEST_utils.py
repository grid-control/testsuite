...
from grid_control.utils.file_tools import GZipTextFile, SafeFile, VirtualFile, with_file_iter
...
class Test_Tarball:
	...
	>>> utils.create_tarball([(None, 'utils/None')], name='test.tar.gz')
	>>> _list_tar() == []
	True
	>>> utils.create_tarball([('TEST_utils.py', 'utils/file')], name='test.tar.gz')
	>>> _list_tar() == [('utils/file', size)]
	True
	>>> utils.create_tarball([(VirtualFile('obj_1', ['Hallo', 'Welt']), 'utils/obj_2')], name='test.tar.gz')
	>>> _list_tar() == [('utils/obj_2', 9)]
	True
	>>> try_catch(lambda: utils.create_tarball([('TEST_utilsX.py', 'utilsX/True')], name='test.tar.gz'), 'PathError', 'does not exist')
	caught
...
class Test_Utils:
	...
	>>> os.environ['PATH'].split(':')[0].endswith('/testsuite/bin')
	True
	>>> 0.012872 <= utils.ping_host('8.8.8.8', timeout=20) <= 0.012874
	0000-00-00 00:00:00 - process.ping:DEBUG - External program called: <testsuite dir>/bin/ping ['-Uqnc', '1', '-W', '20', '8.8.8.8']
	True
